...
std::vector<double> true_vertex_X;  // True origin vertex (cm)
std::vector<double> true_vertex_Y;  // True origin vertex (cm)
std::vector<double> true_vertex_Z;  // True origin vertex (cm)
std::vector<double> true_vertex_T;  // True origin vertex (ns)
std::vector<double> true_dir_X;     // True direction in X (cm)
std::vector<double> true_dir_Y;     // True direction in Y (cm)
std::vector<double> true_dir_Z;     // True direction in Z (cm)
...
int main(int argc, char** argv){
  // DEBUG CHECKING FOR ALPER -- TRIGGERTYPE == -1 COUNTS
  counttypebug = 0;
  ...
  // Read Input Tree
  int nPrimaryEvents = 1000;//fInputTree->GetEntries();
  int iWrite         = 0;

  // Initialize the Histo
  hOpeningAngle = new TH1D(Form("hOpeningAngle"),"",180,0,180);
  hOpeningAngle->GetXaxis()->SetTitle("opening angle (degrees)");
  ...
  for(int i = 0; i < nPrimaryEvents; i++){ 
    ...
    if(fIDevent->GetTrigger(0)->GetTriggerType() == -1){
      std::cout<<"Event "<<i<<" has triggertype -1"<<std::endl;
      counttypebug++;
      continue;
    }else{
  ...
  hOpeningAngle->SetDirectory(fOutputFile);
  hOpeningAngle->Write();
  ...
  std::cout<<counttypebug<<" of "<<nPrimaryEvents<<" has triggertype -1"<<std::endl;
  return 1;
}
...
void SetCustomBranch(TTree* fPrimaryTree) {		
  ...
  fPrimaryTree->Branch("lf_NLL_dir", 		&leaf_output.NLL_dir,		"lf_NLL_dir/D");
...
bool AnalyseEvent(WCSimRootEvent * tEvent, int iEventType) {
  ...
  for(int iTrig = 0; iTrig < tEvent->GetNumberOfEvents(); iTrig++){
	...
	return 0;
      ...
      true_dir_X.push_back(wcTrack->GetDir(0));
      true_dir_Y.push_back(wcTrack->GetDir(1));
      true_dir_Z.push_back(wcTrack->GetDir(2));
    }
						
    // Send True Position to LEAF for check
    std::vector<double> vVtxTrue(3,0.);
    double timeVtxTrue = 0;
    vVtxTrue[0] = true_vertex_X[0];
    vVtxTrue[1] = true_vertex_Y[0];
    vVtxTrue[2] = true_vertex_Z[0];
    timeVtxTrue = true_vertex_T[0];

    std::vector<double> vDIRTrue(3,0.);
    vDIRTrue[0] = true_dir_X[0];
    vDIRTrue[1] = true_dir_Y[0];
    vDIRTrue[2] = true_dir_Z[0];
			
    LEAF::GetME()->SetTrueVertexInfo(vVtxTrue, vDIRTrue, timeVtxTrue);
