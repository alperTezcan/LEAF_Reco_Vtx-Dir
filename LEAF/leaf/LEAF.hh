...
inline bool SortOutputDirVector ( const std::vector<double>& v1, const std::vector<double>& v2 ) { 
	return v1[3] < v2[3]; 
} 
...
class LEAF {
	...
	public:
		...
		double FindNLL_dir_Likelihood(std::vector<double> vertexPosition, std::vector<double> vertexAngles, int nhits);
		void FindNLL_dir_Histo(std::vector<double> vertexPosition, std::vector<double> vertexDirection, int nhits, TH1D * histo);
		...
		double FindNLL_dir_NoLikelihood(std::vector<double> vertexPosition, std::vector<double> vertexAngles, int nhits);
		...
		void SearchDIR_thread(int iStart, int iIte, std::vector<double> vertexPos, int nhits, int tolerance=1);
		...
		void MinimizeDIR_thread(int iStart, int iIte,	
						std::vector< std::vector<double> > coarseDirCandidates,std::vector<double> vertexPos, 
						double stepSize, int nhits, int nCandidates, int tolerance, int verbose);

		void SetTrueVertexInfo(std::vector<double> vtx, std::vector<double> dir, double time);
	...
	private:
		...
		std::vector< std::vector<double> > SearchDIR_Main(std::vector<double> vertexPos, int nhits, int tolerance=1);
		...
		std::vector< std::vector<double> > MinimizeDIR_Main(
						std::vector< std::vector<double> > coarseDirCandidates,	
						std::vector<double> vertexPos, double stepSize, int nhits,
						int nCandidates = 1, int tolerance = 1, int verbose=0);

		std::vector<double> VectorVertexPMT(std::vector<double> vertex, int iPMT);
	...
	private:
		...
		struct SortingNLL_dir { 
			bool operator()(FitDirection const &a, FitDirection const &b) const { 
				return a.NLL_dir < b.NLL_dir;
			}
		};
		...
		TSpline3 * fSplineDIR[NPMT_CONFIGURATION];
		TSpline3 * fSplineCharge[NPMT_CONFIGURATION];
		TSpline3 * fSplineDIR_noDark[NPMT_CONFIGURATION];
		TSpline3 * fSplineCharge_noDark[NPMT_CONFIGURATION];
		...
		std::vector< std::vector<double> > fDirectionList;
